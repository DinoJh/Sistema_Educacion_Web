...
<div class="d-flex align-items-center gap-3 mb-4 flex-wrap">
    ...
    <?php if($puedeEditar): ?>
    <button class="btn btn-primary" onclick="clAbrir('ov-seccion')"><i class="ti-plus me-1"></i>Nueva Sección</button>
    <?php endif; ?>
</div>
...
<div class="mb-3" id="secc-blk-<?=$s->secc_ide?>">
  <div class="seccion-header d-flex align-items-center justify-content-between" onclick="toggleSecc(<?=$s->secc_ide?>)">
    ...
    <div class="d-flex gap-1" onclick="event.stopPropagation()">
        <button class="btn btn-xs" style="padding:2px 8px;font-size:.7rem;background:rgba(124,58,237,.15);color:var(--cl-accent2);" onclick="abrirNuevaLeccion(<?=$s->secc_ide?>,<?=$curso->curs_ide?>)">
  ...
  <div id="body-secc-<?=$s->secc_ide?>" class="mt-1">
    ...
    <div class="leccion-item p-3 d-flex align-items-center gap-3 rounded mb-1">
        ...
        <div class="d-flex align-items-center gap-2 flex-shrink-0">
            ...
            <?php endif; ?>
            <?php if($puedeEditar): ?>
            <button class="btn btn-xs btn-outline-danger" style="padding:2px 6px;font-size:.7rem;" onclick="if(confirm('¿Eliminar lección?'))eliminarLecc(<?=$l->lecc_ide?>)"><i class="ti-trash"></i></button>
            <?php endif; ?>
        </div>
    </div>
    <?php endforeach; ?>
    ...
    <div class="text-center py-3 text-cl-muted small">Sin lecciones en esta sección.
        <?php if($puedeEditar): ?><a href="#" class="text-accent" onclick="abrirNuevaLeccion(<?=$s->secc_ide?>,<?=$curso->curs_ide?>)">+ Agregar</a><?php endif; ?>
    </div>
    <?php endif; ?>
  </div>
</div>
...
<div class="col-lg-4">
    ...
    <div class="card mb-3"><div class="card-body">
        ...
        <button class="btn btn-outline-primary btn-sm w-100 mb-2" onclick="clAbrir('ov-seccion')"><i class="ti-plus me-1"></i>Nueva Sección</button>
        <?php if(!empty($secciones)): ?>
        <button class="btn btn-outline-secondary btn-sm w-100 mb-2" onclick="abrirNuevaLeccion(<?=$secciones[0]->secc_ide?>,<?=$curso->curs_ide?>)"><i class="ti-video-camera me-1"></i>Nueva Lección</button>
        <?php endif; ?>
        <button class="btn btn-outline-warning btn-sm w-100" onclick="clAbrir('ov-editarcurso')"><i class="ti-pencil me-1"></i>Editar Info Curso</button>
    </div></div>
    <?php endif; ?>
</div>
</div>
...
<div class="cl-overlay" id="ov-video">
<div class="cl-modal cl-modal-lg">
  <div class="cl-modal-hdr"><h5 id="ovVideoTitulo">Vista previa</h5><button class="cl-modal-close" onclick="clCerrar('ov-video');document.getElementById('ovVideoFrame').src=''">✕</button></div>
  <div class="cl-modal-body"><div class="video-wrapper"><iframe id="ovVideoFrame" src="" allowfullscreen></iframe></div></div>
</div></div>

<!-- ═══ OVERLAY: NUEVA SECCIÓN ═══ -->
<div class="cl-overlay" id="ov-seccion">
<div class="cl-modal cl-modal-sm">
  <div class="cl-modal-hdr"><h5>Nueva Sección</h5><button class="cl-modal-close" onclick="clCerrar('ov-seccion')">✕</button></div>
  ...
  <div class="cl-modal-ftr">
    <button class="btn btn-secondary btn-sm" onclick="clCerrar('ov-seccion')">Cancelar</button>
    <button class="btn btn-primary btn-sm" onclick="guardarSeccion()">Guardar</button>
  </div>
</div></div>

<!-- ═══ OVERLAY: EDITAR CURSO ═══ -->
<div class="cl-overlay" id="ov-editarcurso">
<div class="cl-modal cl-modal-lg">
  <div class="cl-modal-hdr"><h5><i class="ti-pencil me-2"></i>Editar Curso</h5><button class="cl-modal-close" onclick="clCerrar('ov-editarcurso')">✕</button></div>
  <div class="cl-modal-body">
    <div class="row g-3">
  ...
  <div class="cl-modal-ftr">
    <button class="btn btn-secondary btn-sm" onclick="clCerrar('ov-editarcurso')">Cancelar</button>
    <button class="btn btn-primary btn-sm" onclick="guardarEditCurso()"><i class="ti-save me-1"></i>Guardar</button>
  </div>
</div></div>

<!-- ═══ OVERLAY: NUEVA LECCIÓN ═══ -->
<div class="cl-overlay" id="ov-leccion">
<div class="cl-modal cl-modal-lg">
  <div class="cl-modal-hdr"><h5><i class="ti-video-camera me-2 text-accent"></i>Nueva Lección</h5><button class="cl-modal-close" onclick="clCerrar('ov-leccion')">✕</button></div>
  <div class="cl-modal-body">
    ...
    <div class="row g-3">
      ...
      <div id="campoUrl" class="col-12">
        <label class="form-label small text-cl-muted">URL del video</label>
        <div class="d-flex gap-2">
            <input type="text" id="lUrl" class="form-control" placeholder="https://youtube.com/watch?v=…">
      ...
      <div id="campoArchivo" class="col-12" style="display:none;">
        <label class="form-label small text-cl-muted">URL del archivo</label>
        <input type="text" id="lArchivoUrl" class="form-control" placeholder="https://…"></div>
      <div class="col-12"><label class="form-label small text-cl-muted">Descripción breve</label>
        <textarea id="lDesc" class="form-control" rows="2"></textarea></div>
      <div class="col-md-6"><div class="form-check mt-2">
        <input class="form-check-input" type="checkbox" id="lPreview">
        <label class="form-check-label small text-cl-muted" for="lPreview">Vista previa gratuita</label>
      </div></div>
  ...
  <div class="cl-modal-ftr">
    <button class="btn btn-secondary btn-sm" onclick="clCerrar('ov-leccion')">Cancelar</button>
    <button class="btn btn-primary btn-sm" onclick="guardarLeccion()"><i class="ti-save me-1"></i>Guardar Lección</button>
  </div>
</div></div>
...
function previewVideo(url,titulo,tipo){
    ...
    document.getElementById('ovVideoTitulo').innerHTML=titulo;
    document.getElementById('ovVideoFrame').src=e||url;
    clAbrir('ov-video');
}
function abrirNuevaLeccion(seccIde,cursIde){
...
function previsualizarEmbed(){
    ...
    var e=embedUrl(url);
    if(!e){alertar('URL no reconocida.','alert alert-warning','ti-alert');return;}
...
function guardarSeccion(){
    ...
    if(!n){alertar('Escribe el nombre de la sección.','alert alert-warning','ti-alert');return;}
    clCerrar('ov-seccion');
    openCargar();
    $.post("<?=base_url('/secciones/guardar')?>",{curs_ide:cursIdeActual,nombre:n},function(){
        closeCargar();
        recargarCurso(cursIdeActual);
    });
}
function guardarEditCurso(){
    clCerrar('ov-editarcurso');
    openCargar();
    $.post("<?=base_url('/cursos/actualizar')?>",{
        ...
        setTimeout(()=>recargarCurso(cursIdeActual),900);
    });
}
function guardarLeccion(){
    var t=document.getElementById('lTitulo').value.trim();
    if(!t){alertar('Escribe el título de la lección.','alert alert-warning','ti-alert');return;}
    clCerrar('ov-leccion');
    openCargar();
...
function eliminarSecc(ide){
    openCargar();
    $.post("<?=base_url('/secciones/eliminar')?>",{ide:ide},function(){
        closeCargar();
        recargarCurso(cursIdeActual);
    });
}
function eliminarLecc(ide){
    openCargar();
    $.post("<?=base_url('/lecciones/eliminar')?>",{ide:ide},function(){
        closeCargar();
        recargarCurso(cursIdeActual);
    });
}
function recargarCurso(ide){cargarFuncion('/cursos/ver/'+ide,'Cursos','Gestionar Curso','Secciones y lecciones');}
</script>
