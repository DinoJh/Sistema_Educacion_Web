...
// Organizar lecciones por seccion
$lecBySecc = [];
foreach($lecciones as $l) {
    $lecBySecc[$l->lecc_secc_ide ?? 0][] = $l;
}
?>
...
<div class="text-center py-5 card"><div class="card-body">
    ...
    <p class="mt-2 text-cl-muted">No hay secciones aún. Crea la primera sección para empezar.</p>
</div></div>
<?php endif; ?>
<?php foreach($secciones as $idx => $s): ?>
<div class="mb-3" id="secc-<?=$s->secc_ide?>">
  <div class="seccion-header d-flex align-items-center justify-content-between" onclick="toggleSecc(<?=$s->secc_ide?>)">
    ...
    <div class="d-flex gap-2" onclick="event.stopPropagation()">
        <button class="btn btn-xs btn-outline-primary" style="padding:2px 8px;font-size:.7rem;" onclick="abrirModalLeccion(<?=$s->secc_ide?>,<?=$curso->curs_ide?>)"><i class="ti-plus"></i> Lección</button>
        <button class="btn btn-xs btn-outline-danger" style="padding:2px 8px;font-size:.7rem;" onclick="if(confirm('Eliminar sección y sus lecciones?'))eliminarSecc(<?=$s->secc_ide?>)"><i class="ti-trash"></i></button>
    </div>
    <?php endif; ?>
  </div>
  <div id="body-secc-<?=$s->secc_ide?>" class="mt-1">
    <?php foreach($lecBySecc[$s->secc_ide]??[] as $l): ?>
    <div class="leccion-item p-3 d-flex align-items-center gap-3 rounded mb-1" id="lecc-<?=$l->lecc_ide?>">
        ...
        <div class="flex-grow-1">
            ...
            <?php endif; ?>
            <?php if($l->lecc_url): ?>
            <div class="mt-1"><small class="text-accent" style="word-break:break-all;"><?=htmlspecialchars($l->lecc_url)?></small></div>
            <?php endif; ?>
        </div>
        <div class="d-flex align-items-center gap-2">
            <?php if($l->lecc_es_preview): ?><span class="badge" style="background:rgba(6,182,212,.15);color:#06b6d4;font-size:.65rem;">PREVIEW</span><?php endif; ?>
            <?php if($l->lecc_tipo=='VIDEO'&&$l->lecc_url): ?>
            <button class="btn btn-xs" style="padding:3px 8px;background:rgba(124,58,237,.15);color:var(--cl-accent2);font-size:.7rem;" onclick="previewVideo('<?=htmlspecialchars($l->lecc_url?>','<?=htmlspecialchars($l->lecc_titulo??'')?>')">▶ Ver</button>
            <?php endif; ?>
            <?php if($puedeEditar): ?>
            <button class="btn btn-xs btn-outline-danger" style="padding:2px 6px;font-size:.7rem;" onclick="if(confirm('Eliminar?'))$.post('<?=base_url('/lecciones/eliminar')?>',{ide:<?=$l->lecc_ide?>},function(){recargarCurso(<?=$curso->curs_ide?>);})"><i class="ti-trash"></i></button>
            <?php endif; ?>
        </div>
    </div>
    <?php endforeach; ?>
    <?php if(empty($lecBySecc[$s->secc_ide]??[])): ?>
    <div class="text-center py-3 text-cl-muted small">Sin lecciones en esta sección.</div>
    <?php endif; ?>
  </div>
</div>
<?php endforeach; ?>
...
<div class="col-lg-4">
    ...
    <div class="card"><div class="card-body">
        ...
        <?php if(!empty($secciones)): ?>
        <button class="btn btn-outline-secondary btn-sm w-100" onclick="abrirModalLeccion(<?=$secciones[0]->secc_ide?>,<?=$curso->curs_ide?>)"><i class="ti-video-camera me-1"></i>Nueva Lección</button>
        <?php endif; ?>
    </div></div>
    <?php endif; ?>
</div>
...
<div class="modal-dialog modal-xl"><div class="modal-content" style="background:var(--cl-bg-card);">
  ...
  <button class="btn-close btn-close-white" data-bs-dismiss="modal" onclick="document.getElementById('videoFrame').src=''"></button></div>
...
<div class="modal-dialog"><div class="modal-content" style="background:var(--cl-bg-card);">
  ...
  <div class="modal-body">
    <input type="hidden" id="seccCursIde" value="<?=$curso->curs_ide??0?>">
...
<div class="modal-dialog modal-lg"><div class="modal-content" style="background:var(--cl-bg-card);">
  ...
  <div class="modal-body">
    ...
    <div class="row g-3">
      ...
      <div id="campoUrl" class="col-12"><label class="form-label small text-cl-muted">URL del video (YouTube o Google Drive)</label>
        ...
        <button class="btn btn-sm btn-outline-secondary mt-2" type="button" onclick="previsualizarEmbed()"><i class="ti-eye me-1"></i>Previsualizar</button>
      </div>
      <div id="campoArchivo" class="col-12" style="display:none;"><label class="form-label small text-cl-muted">URL del Archivo</label>
        <input type="text" id="lArchivoUrl" class="form-control" placeholder="/archivos/guia.pdf o URL externa"></div>
      <div class="col-12"><label class="form-label small text-cl-muted">Descripción</label>
        <textarea id="lDesc" class="form-control" rows="2" placeholder="Descripción breve de la lección"></textarea></div>
  ...
  <div class="modal-footer border-0">
    <button class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
    <button class="btn btn-primary px-4" onclick="guardarLeccion()"><i class="ti-save me-1"></i>Guardar</button>
  </div>
</div></div></div>
...
function toggleSecc(ide){
    ...
    ic.className=b.style.display=='none'?'ti-angle-right':'ti-angle-down';
}
function previewVideo(url,titulo){
    var e=embedUrl(url);
...
function previsualizarEmbed(){
    ...
    var e=embedUrl(url);
    if(!e){alertar('URL no válida','alert alert-warning');return;}
...
function guardarLeccion(){
    ...
    if(!t){alertar('Escribe el título.','alert alert-warning');return;}
