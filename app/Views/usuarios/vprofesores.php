<div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
    ...
    <button class="btn btn-primary" onclick="abrirModalNuevo(2)"><i class="ti-plus me-1"></i>Nuevo Profesor</button>
</div>
...
<tr>
    ...
    <td>
        ...
        <button class="btn btn-xs btn-outline-danger" style="padding:2px 8px;font-size:.7rem;" onclick="if(confirm('Eliminar?'))$.post('<?=base_url('/usuarios/eliminar')?>',{ide:<?=$u->usua_ide?>},function(r){r=JSON.parse(r);if(r.ok){alertar(r.msg,'alert alert-success');setTimeout(()=>cargarFuncion('/usuarios/profesores','Usuarios','Profesores',''),1000);}})">Eliminar</button>
    </td>
</tr>
<?php endforeach; ?>
...
function cambiarEstado(ide,esta){
    $.post("<?=base_url('/usuarios/cambiarestado')?>",{ide:ide,esta:esta},function(){
        alertar('Estado actualizado.','alert alert-success');setTimeout(()=>cargarFuncion('/usuarios/profesores','Usuarios','Profesores',''),900);
    });
}
