...
class Panel extends BaseController
{
    ...
    // MIS CURSOS: solo cursos en los que está matriculado
    public function cursos()
    {
        $db = \Config\Database::connect();
        $usua_ide = $this->session->usua_ide;
        $matriculas = $db->table('matriculas m')
            ->select('c.curs_ide, c.curs_nombre, c.curs_nivel, c.curs_descripcion,
                cat.cate_nombre, u.usua_nombres, u.usua_paterno,
    ...
    // CATÁLOGO: todos los cursos disponibles para buscar y matricularse
    public function catalogo()
    {
        ...
        $cursos = $db->table('cursos c')
            ->select('c.*, cat.cate_nombre, u.usua_nombres, u.usua_paterno,
                ...
                (SELECT matr_ide FROM matriculas m2 WHERE m2.matr_curs_ide=c.curs_ide AND m2.matr_usua_ide='.$usua_ide.' LIMIT 1) as ya_matriculado')
            ->join('categorias cat','cat.cate_ide=c.curs_cate_ide','left')
            ->join('profesores p','p.prof_ide=c.curs_prof_ide','left')
            ->join('usuarios u','u.usua_ide=p.prof_usua_ide','left')
    ...
    public function ver($curs_ide)
    {
        ...
        // Solo VIDEO y ARCHIVO - sin QUIZ
        $lecciones = $db->table('lecciones')
            ->where('lecc_curs_ide',$curs_ide)
    ...
    public function matricular()
    {
        ...
        $usua_ide = $this->session->usua_ide;
        $db = \Config\Database::connect();
        $existe = $db->table('matriculas')->where(['matr_usua_ide'=>$usua_ide,'matr_curs_ide'=>$curs_ide])->get()->getRow();
        if (!$existe) {
            General::insertar('matriculas',['matr_usua_ide'=>$usua_ide,'matr_curs_ide'=>$curs_ide,'matr_fecha'=>date('Y-m-d H:i:s'),'matr_esta_ide'=>1]);
        }
        echo json_encode(['ok'=>true,'msg'=>'¡Te has inscrito exitosamente!']);
    }
    ...
    public function marcar()
    {
        ...
        if ($lecc) {
            $total  = $db->table('lecciones')->where(['lecc_curs_ide'=>$lecc->lecc_curs_ide,'lecc_esta_ide'=>1])->whereNotIn('lecc_tipo',['QUIZ'])->countAllResults();
            $hechas = $db->table('progreso p')->join('lecciones l','l.lecc_ide=p.prog_lecc_ide')->where(['p.prog_usua_ide'=>$usua_ide,'l.lecc_curs_ide'=>$lecc->lecc_curs_ide,'p.prog_completado'=>1])->countAllResults();
    ...
    public function progreso()
    {
        ...
        $matriculas = $db->table('matriculas m')
            ->select('m.*, c.curs_nombre, c.curs_nivel, c.curs_ide,
                (SELECT COUNT(*) FROM lecciones l WHERE l.lecc_curs_ide=c.curs_ide AND l.lecc_esta_ide=1 AND l.lecc_tipo!="QUIZ") as total_lecc,
                (SELECT COUNT(*) FROM progreso pr JOIN lecciones ll ON ll.lecc_ide=pr.prog_lecc_ide WHERE pr.prog_usua_ide=m.matr_usua_ide AND ll.lecc_curs_ide=c.curs_ide AND pr.prog_completado=1) as lecc_hechas,
