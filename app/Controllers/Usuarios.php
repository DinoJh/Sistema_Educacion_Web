...
class Usuarios extends BaseController
{
    ...
    public function alumnos()
    {
        ...
        $alumnos = $db->table('usuarios u')
            ->select('u.usua_ide, u.usua_dni, u.usua_nombres, u.usua_paterno, u.usua_materno, u.usua_email, u.usua_celular, u.usua_user, e.esta_nombre, e.esta_clase,
            ...
            ->get()->getResult();
        echo view('usuarios/valumnos', ['usuarios'=>$alumnos,'base'=>base_url('public'),'session'=>$this->session]);
    }
    ...
    public function profesores()
    {
        ...
        $profes = $db->table('usuarios u')
            ->select('u.usua_ide, u.usua_dni, u.usua_nombres, u.usua_paterno, u.usua_materno, u.usua_email, u.usua_user, e.esta_nombre, e.esta_clase, p.prof_especialidad, p.prof_grado,
            ...
            ->orderBy('u.usua_paterno')
            ->get()->getResult();
        echo view('usuarios/vprofesores', ['usuarios'=>$profes,'base'=>base_url('public'),'session'=>$this->session]);
    }

    public function eliminar()
    {
        $ide = $this->request->getPost('ide');
        General::actualizar('usuarios',['usua_ide'=>$ide],['usua_deleted_at'=>date('Y-m-d H:i:s'),'usua_esta_ide'=>2]);
    ...
    public function nuevo()
    {
        ...
        $db = \Config\Database::connect();
        $existe = $db->table('usuarios')->where('usua_user',$data['usua_user'])->orWhere('usua_email',$data['usua_email'])->get()->getRow();
        if ($existe) { echo json_encode(['ok'=>false,'msg'=>'Usuario o email ya existe.']); return; }
        General::insertar('usuarios',$data);
        $uid = $db->insertID();
        if ($perf == 2) { // Crear registro de profesor
